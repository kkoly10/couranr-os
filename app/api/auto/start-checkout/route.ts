...
export async function POST(req: NextRequest) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // ✅ FIX: you have /auto/success, not /auto/checkout/success
      success_url: `${siteUrl}/auto/success?rentalId=${encodeURIComponent(rental.id)}`,
      cancel_url: `${siteUrl}/auto/checkout?rentalId=${encodeURIComponent(rental.id)}`,
    });

    await supabaseAdmin.from("rentals").update({ stripe_checkout_session_id: session.id }).eq("id", rental.id);
    ...
    if (!session.url) return NextResponse.json({ error: "Checkout session URL not returned by Stripe" }, { status: 500 });
