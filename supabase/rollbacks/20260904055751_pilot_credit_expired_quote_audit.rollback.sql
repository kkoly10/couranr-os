-- Roll back 20260904055751_pilot_credit_expired_quote_audit.
-- Preserve immutable evidence: if an applied credit actually used the
-- expired-quote exception, removing the audit bit would make that fact vanish.
begin;
do $$
begin
  if exists(
    select 1 from public.couranr_promotional_credits
     where quote_expired_at_credit=true
  ) then
    raise exception 'refusing to remove expired-quote audit evidence';
  end if;
end $$;
...
alter table public.couranr_promotional_credits
  drop column if exists quote_expired_at_credit;
commit;
