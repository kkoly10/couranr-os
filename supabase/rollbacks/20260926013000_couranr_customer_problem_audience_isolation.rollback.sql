-- Paired rollback for CUS-004 audience isolation.
-- Refuse once sender/recipient audience-scoped report history exists: collapsing
-- those cases back to one delivery-level namespace would destroy privacy facts.
begin;
...
do $$ begin
  if exists (
    select 1 from public.couranr_customer_problem_reports
     where customer_audience <> 'legacy'
  ) then
    raise exception 'customer_problem_audience_rollback_refuses_semantic_use';
  end if;
end $$;
...
drop index public.couranr_cpr_one_draft_per_delivery_audience_uniq;
drop index public.couranr_cpr_one_open_per_delivery_audience_uniq;
drop index public.couranr_cpr_submit_key_audience_uniq;
create unique index couranr_cpr_one_draft_per_delivery_uniq
  on public.couranr_customer_problem_reports(delivery_id)
  where report_state='draft';
create unique index couranr_cpr_one_open_per_delivery_uniq
  on public.couranr_customer_problem_reports(delivery_id)
  where report_state<>'resolved';
create unique index couranr_cpr_submit_key_uniq
...
alter table public.couranr_customer_problem_reports
  drop constraint couranr_cpr_customer_audience_chk;
alter table public.couranr_customer_problem_reports
  drop column customer_audience;
