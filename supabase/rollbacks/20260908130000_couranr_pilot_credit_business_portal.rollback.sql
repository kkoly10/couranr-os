-- Roll back merchant_portal eligibility for Couranr pilot credits.
--
-- Once this migration has actually funded a merchant_portal request, removing
-- replay eligibility would make a committed commercial action non-replayable.
-- Refuse destructive semantic rollback and require roll-forward instead.
begin;
...
begin
  if exists (
    ...
    join public.couranr_delivery_requests r on r.id=c.request_id
    where r.source='merchant_portal'
  ) then
    raise exception 'rollback_refused: merchant_portal promotional credit evidence exists';
  end if;
end $$;
