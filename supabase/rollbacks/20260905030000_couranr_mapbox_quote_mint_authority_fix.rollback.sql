-- Roll back only the quote-mint authority correction.
-- This restores the pre-fix Google-only appender and is intended only with an
-- application rollback that no longer emits Mapbox runtime evidence.
begin;
