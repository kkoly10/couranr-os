-- Roll back the CUS-004 turn-preservation and evidence-arrival guards.
--
-- Restores couranr_transition_customer_problem_report to the definition
-- 20260908160000 installed, which is what production ran before the forward
-- migration. Extracted from that file rather than retyped, so the rollback
-- provably returns the database to a state the repository already describes.
--
-- Re-runnable: `create or replace` with no dependency on current state.
--
-- SEMANTIC COST OF ROLLING BACK, stated rather than discovered later. The prior
-- definition (a) clears the conversation turn unconditionally, so an unrelated
-- newer customer Help message can lose its outstanding turn, and (b) lets
-- Operations move awaiting_evidence -> under_review before the requested photos
-- arrive, removing the customer's upload UI. Rolling back reinstates both.
