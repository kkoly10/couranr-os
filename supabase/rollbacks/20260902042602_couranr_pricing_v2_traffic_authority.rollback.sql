...
-- it removes the traffic columns and their constraints, drops the V2 function
-- arities, and re-creates the Batch 1 arities verbatim so the database is left
-- able to mint quotes rather than stranded with no quote commands at all.
-- =====================================================================
...
begin
  -- Every other statement in this file is defensive (drop ... if exists). This
  -- one runs FIRST and reads a V2 column, so without the existence check a
  -- re-run, or a run against a database the migration never reached, aborts
  -- with a bare 42703 undefined_column naming a Couranr column - which reads
  -- as schema corruption rather than "nothing to do".
  select exists (
    select 1 from information_schema.columns
     where table_schema='public' and table_name='couranr_quote_versions'
       and column_name='route_traffic_delay_seconds'
  ) into v_has_traffic;
  ...
  if v_has_traffic then
    execute 'select count(*) from public.couranr_quote_versions where route_traffic_delay_seconds is not null'
      into v_traffic_rows;
  else
    v_traffic_rows := 0;
  end if;
...
-- No V2 evidence exists, so the added surface can be removed cleanly.
--
-- Re-running the Batch 1 migration does NOT restore the functions and must not
-- be relied on: this rollback deliberately KEEPS route_distance_meters and
-- serviceability_outcome (they are Batch 1's, not V2's), and Batch 1 opens with
-- a guard that raises 'Business routing authority columns already exist;
-- refusing an unknown partial application' when it finds them. So the recovery
-- this file used to point at aborts on its first statement. The Batch 1
-- definitions are re-created at the bottom of this file instead, verbatim.
...
begin
  ...
  v_quote := private.couranr_append_routed_quote_version(
    ...
    p_route_duration_seconds,p_distance_source,p_serviceability_outcome,
...
comment on function public.couranr_create_routed_delivery_request_draft is
  'Creates a Business request and immutable quote from Google Place snapshots and server-only Routes v2 evidence.';
comment on function public.couranr_calculate_routed_delivery_request_estimate is
  'CAS re-estimate command. Address edits create a new immutable quote using fresh server-only Routes v2 evidence.';
comment on function public.couranr_create_routed_quote_version is
  'General CAS quote append using fresh server-only Routes v2 evidence; supersedes the pre-routing quote command.';
comment on function public.couranr_requote_routed_delivery_request is
  'Operations requote using fresh server-only Routes v2 evidence and the existing deterministic pricing policy.';
