...
do $$ begin
  if exists(select 1 from public.couranr_driver_reviews)
     ...
     or exists(select 1 from private.couranr_ledger_transactions
               where source_kind in ('tip','tip_refund','tip_dispute_loss')) then
...
drop function public.couranr_settle_driver_tip(uuid,text,uuid,uuid,text,integer,integer,integer,text,text,text,integer);
drop function public.couranr_replace_driver_tip_intent(uuid,text,integer,text);
...
begin
  ...
  if position('p_source_kind not in (''capture'',''refund'',''cancellation_receivable'',''tip'',''tip_refund'',''tip_dispute_loss'')' in v_definition)=0 then
