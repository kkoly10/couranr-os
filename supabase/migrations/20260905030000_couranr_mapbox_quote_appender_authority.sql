-- ============================================================================
-- Mapbox quote-appender authority repair
--
-- Fresh regression evidence on main @ 1de280bc978b3f4d7a5cc144421bfaece419f228:
-- the application and 20260904175646 cut routing over to
-- mapbox_directions_v5, but the canonical private quote appender still
-- rejected every source except google_routes_v2. That makes a valid Mapbox
-- route fail at the immutable quote mint boundary.
--
-- This is a narrow repair, not a routing redesign:
-- * Google Places remains address identity.
-- * Mapbox Directions v5 / driving-traffic is the only NEW route authority.
-- * Historical Google quote evidence remains untouched and valid.
-- ============================================================================
...
begin
  ...
  insert into public.couranr_quote_versions(
    ...
    jsonb_build_object('serviceLevel',v_req.service_level,
                       ...
                       'routeAuthority','mapbox_directions_v5',
