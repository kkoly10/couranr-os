-- ============================================================================
-- Mapbox quote-mint authority correction
--
-- Fresh regression evidence on 2026-09-05 showed the runtime router emits
-- distance_source='mapbox_directions_v5' while the final quote appender still
-- required 'google_routes_v2'. That made every new Mapbox-backed quote fail
-- closed after the routing cutover. Historical Google quote evidence remains
-- immutable and valid; this changes only the runtime MINT boundary.
-- ============================================================================
...
begin
  if to_regprocedure('private.couranr_append_routed_quote_version( uuid,uuid,integer,text,text,integer,integer,numeric,jsonb,jsonb, bigint,integer,integer,integer,text,text,text )') is null then
    raise exception 'Mapbox quote authority fix requires the canonical routed quote appender';
  end if;
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.couranr_quote_versions'::regclass
      and conname='couranr_qv_route_evidence_chk'
      and pg_get_constraintdef(oid) like '%mapbox_directions_v5%'
  ) then
    raise exception 'Mapbox quote evidence constraint is not active';
  end if;
end
$guard$;
...
begin
  ...
  if p_distance_source is distinct from 'mapbox_directions_v5'
     or p_serviceability_outcome not in ('available_for_request','needs_review') then
    raise exception 'mapbox_route_authority_required' using errcode='CR422';
  end if;
  ...
  if p_serviceability_outcome='available_for_request' then
    if p_route_distance_meters is null or p_route_distance_meters < 0
      ...
      raise exception 'complete_mapbox_route_evidence_required' using errcode='CR422';
    end if;
  ...
  insert into public.couranr_quote_versions(
    ...
    jsonb_build_object('serviceLevel',v_req.service_level,
                       ...
                       'routeAuthority',p_distance_source,
...
comment on function private.couranr_append_routed_quote_version(
  uuid,uuid,integer,text,text,integer,integer,numeric,jsonb,jsonb,
  bigint,integer,integer,integer,text,text,text
) is
  'Runtime quote mint boundary: Google Places owns address identity; Mapbox Directions v5 driving-traffic owns distance, duration and traffic. Historical Google route evidence is unchanged.';
