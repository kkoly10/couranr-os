...
-- switch is armed. Even then, SESSION CREATION is limited to one explicitly
-- issued production-canary relationship. Raw access codes, cookie secrets and
-- guest tokens are never persisted; only SHA-256 hashes are stored.
--
-- This is release-control evidence, not product tenancy. RLS is deny-all and
-- every command is service_role only. Paid-provider entrypoints are capped per
-- canary before the existing global paid-provider budget is consulted.
...
comment on table public.couranr_consumer_canary_access is
  'Hash-only production /send canary access. One active canary relationship may mint one guest session. Does not change request tenancy or commercial authority.';

alter table public.couranr_consumer_canary_access enable row level security;
revoke all on public.couranr_consumer_canary_access
  from public,anon,authenticated,service_role;
grant select,insert,update on public.couranr_consumer_canary_access
  to service_role;
...
begin
  ...
  -- The product contract is ONE production canary at a time. Serialize issue
  -- attempts so concurrent callers cannot both observe an empty active set.
...
begin
  ...
  -- Reuse the canonical guest-session issuer rather than forking session
  -- authority. The canary can only shorten the canonical expiry.
