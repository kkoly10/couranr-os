-- =====================================================================
-- COURANR HOSTED LEGACY VALIDATE — DEPLOY-GAP FAIL-CLOSED GUARD (PREDEPLOY)
--
-- Companion to 20260908220000, found by adversarial review of that migration.
-- 20260908220000 RETAINS the old 26-argument couranr_validate_hosted_delivery_
-- request verbatim for the zero-downtime deploy gap. Verbatim is one line too
-- faithful: the v1 body writes timing_intent='asap' and never touches
-- requested_pickup_local / requested_departure_at. Before 20260908220000 no
-- hosted row could be scheduled, so that was harmless; now a scheduled request
-- created by the NEW application and validated by a still-serving OLD
-- application would be rewritten to asap with a stale local time and instant,
-- and its quote minted against a route computed for the wrong departure.
--
-- This redefines the SAME 26-argument arity (signature unchanged, so PostgREST
-- resolution for the old application is unchanged) with one fail-closed guard:
-- a scheduled row raises CR409 hosted_scheduled_timing_requires_current_
-- application. Rows the old application itself creates are always asap, so
-- the guard never fires on the old application's own traffic — production
-- keeps working exactly as today when this is applied PREDEPLOY.
--
-- The POSTDEPLOY fence 20260908230000 still drops this arity; its rollback
-- restores THIS guarded body, not the v1 one. The full rollback of
-- 20260908220000 restores v1 verbatim — safe there because its evidence guard
-- proves no scheduled hosted row exists.
--
-- ADDITIVE: one function body replaced in place. RE-RUNNABLE: create-or-replace,
-- and a no-op once the fence has retired the arity.
-- Rollback: supabase/rollbacks/20260908220500_couranr_hosted_legacy_validate_guard.rollback.sql
-- =====================================================================
...
/* Body verbatim from 20260905040000 plus the DEPLOY-GAP guard. Wrapped so the
   redefinition is skipped when the fence has already dropped the arity — this
   migration never resurrects a retired legacy shape. */
do $redefine$
begin
