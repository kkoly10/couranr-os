-- =====================================================================
-- FAST TRACK LAUNCH / BATCH 1
-- Canonical Google address and route authority for Couranr Business.
--
-- Additive schema, then an explicit command cutover. Historical quote rows
-- are not rewritten: their provenance and prior distance_source remain exact.
-- The two pre-routing Business RPCs remain installed for audit/compatibility
-- inspection but lose service_role EXECUTE so they cannot mint a new quote.
-- =====================================================================
...
begin
  if to_regclass('public.couranr_quote_versions') is null
     ...
     or to_regprocedure('private.couranr_append_quote_version(uuid,uuid,integer,text,text,integer,integer,numeric,jsonb,jsonb)') is null
     or not exists (
       select 1 from information_schema.columns
        where table_schema='public' and table_name='couranr_delivery_requests'
          and column_name='single_destination_contract'
     ) then
    raise exception 'Business routing authority requires Foundation Gate A M1-M6';
  end if;
  if exists (
    select 1 from information_schema.columns
     where table_schema='public' and table_name='couranr_quote_versions'
       and column_name in ('route_distance_meters','serviceability_outcome')
  ) then
    raise exception 'Business routing authority columns already exist; refusing an unknown partial application';
  end if;
end
$guard$;

alter table public.couranr_quote_versions
  add column route_distance_meters bigint,
  add column serviceability_outcome text,
  add constraint couranr_qv_route_meters_chk check (
    route_distance_meters is null or route_distance_meters >= 0
  ),
  add constraint couranr_qv_serviceability_outcome_chk check (
    serviceability_outcome is null
    or serviceability_outcome in ('available_for_request','needs_review')
  ),
  add constraint couranr_qv_google_route_evidence_chk check (
    serviceability_outcome is null
    or (distance_source = 'google_routes_v2' and (
...
-- Only routed commands may mint new Business request quotes after
-- this cutover. The old functions remain queryable as historical DDL.
