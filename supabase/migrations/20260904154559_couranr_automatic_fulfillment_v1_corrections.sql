-- Automatic Fulfillment V1 correction:
-- promotional credit is an alternative settlement authority, not fake payment history.
begin;
set local statement_timeout='120s';
set local lock_timeout='10s';

alter table public.couranr_service_plans
  alter column payment_obligation_id drop not null;
...
alter table public.couranr_service_plans
  add constraint couranr_sp_settlement_identity_chk check (
    (promotional_credit_id is null and payment_obligation_id is not null)
    or
    (promotional_credit_id is not null and payment_obligation_id is null)
  );

alter table public.couranr_deliveries
  drop constraint if exists couranr_dlv_settlement_identity_chk;
alter table public.couranr_deliveries
  add constraint couranr_dlv_settlement_identity_chk check (
    (promotional_credit_id is null and payment_obligation_id is not null)
    or
    (promotional_credit_id is not null and payment_obligation_id is null)
  );
...
-- Manual intervention resolves the exact machine exception it replaces.
create or replace function private.couranr_resolve_manual_plan_exception()
returns trigger language plpgsql set search_path=''
as $fn$
begin
  if new.plan_state='confirmed' and new.plan_source='operations' then
    update public.couranr_automation_exceptions
       set exception_state='resolved',resolved_at=now(),last_seen_at=now()
     where request_id=new.request_id
       and exception_stage='planning'
...
drop trigger if exists couranr_resolve_manual_plan_exception
  on public.couranr_service_plans;
create trigger couranr_resolve_manual_plan_exception
after insert on public.couranr_service_plans
for each row execute function private.couranr_resolve_manual_plan_exception();

create or replace function private.couranr_resolve_manual_dispatch_exception()
returns trigger language plpgsql set search_path=''
as $fn$
declare
  v_request_id uuid;
begin
  if new.assignment_source='operations' and new.assignment_state='active' then
...
drop trigger if exists couranr_resolve_manual_dispatch_exception
  on public.couranr_delivery_assignments;
create trigger couranr_resolve_manual_dispatch_exception
after insert on public.couranr_delivery_assignments
for each row execute function private.couranr_resolve_manual_dispatch_exception();

-- Exact Operations work candidates. Normal automatic schedules are excluded;
-- open automation exceptions are always included.
